...
all_2016 = pd.read_csv("C:\\Users\\Austin\\Documents\\Baseball\\savant_data (6).csv").replace('null', np.nan)
...
pitch = all_2016[['pitch_type', 'balls', 'strikes']].dropna()
pd.to_numeric(pitch.balls)
pd.to_numeric(pitch.strikes)

for index, row in pitch.iterrows():
    if row['pitch_type'] == 'Single':
        singles += 1
        hits += 1
